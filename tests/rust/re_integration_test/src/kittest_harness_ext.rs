...
pub trait HarnessExt<'h> {
    ...
    // Initializes the chunk store with a new, empty recording but no blueprint.
    // This simulates the real SDK flow where only recording data arrives.
    fn init_recording_without_blueprint(&mut self);
...
impl<'h> HarnessExt<'h> for egui_kittest::Harness<'h, re_viewer::App> {
    ...
    fn init_recording_without_blueprint(&mut self) {
        ...
        // No blueprint created or registered, the viewer should handle this itself.
