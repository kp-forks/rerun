...
static REQUIRED_COMPONENTS: std::sync::LazyLock<[ComponentDescriptor; 1usize]> =
    std::sync::LazyLock::new(|| [StateChange::descriptor_state()]);
...
impl ::re_types_core::AsComponents for StateChange {
    #[inline]
...
impl ::re_types_core::ArchetypeReflectionMarker for StateChange {}

impl crate::VisualizableArchetype for StateChange {
    #[inline]
...
impl StateChange {
        ...
        I: IntoIterator<Item = usize> + Clone,
    {
        let columns = [self
            .state
            .map(|state| state.partitioned(_lengths.clone()))
            .transpose()?];
    ...
    pub fn columns_of_unit_batches(
        ...
        let len_state = self.state.as_ref().map(|b| b.array.len());
        let len = None.or(len_state).unwrap_or(0);
