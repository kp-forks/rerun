...
def first_segment_store(readonly_test_dataset: DatasetEntry) -> ChunkStore:
    """The `ChunkStore` for the first segment in [`readonly_test_dataset`][]."""
...
def test_segment_store_basic(first_segment_store: ChunkStore) -> None:
    assert isinstance(first_segment_store, ChunkStore)
...
def test_segment_store_summary_forces_load(first_segment_store: ChunkStore) -> None:
    """`summary()` calls `collect_all_chunks` → `lazy.load_all_chunks` → trait impl."""
    summary = first_segment_store.summary()
    assert summary
    assert "rows=" in summary


def test_segment_store_stream_to_chunks(first_segment_store: ChunkStore) -> None:
...
def test_segment_store_write_rrd_roundtrip(single_segment_store: ChunkStore, tmp_path: Path) -> None:
...
def test_segment_store_compile_twice_works(first_segment_store: ChunkStore) -> None:
